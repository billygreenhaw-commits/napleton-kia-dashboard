...
function estimateNpsDistribution(completed, nps){
  ...
  let promoters = Math.max(0, Math.min(total, net))
  let detractors = Math.max(0, promoters - net)
  let passives = Math.max(0, total - promoters - detractors)

  // If net is positive, this keeps NPS mathematically correct but cannot know the exact Kia P/P/D split from this report alone.
  if (net >= 0) {
    promoters = net
    detractors = 0
    passives = Math.max(0, total - promoters)
  }
...
function parseCsiWorkbook(workbook){
  ...
  for (const row of raw) {
    ...
    if (lower === 'national') {
      metrics.national_score = nps
      continue
    }

    if (lower === 'ce') {
      metrics.region_score = nps
      continue
    }

    if (lower === 'mo027' || lower.includes('napleton')) {
      metrics.store_nps = nps
      continue
    }
    ...
    advisorRows.push({
      advisor: titleCaseAdvisor(name),
      nps,
      completed_surveys: completed,
      promoters: dist.promoters,
      passives: dist.passives,
  ...
  async function handleCsiUpload(e){
    ...
    try{
      ...
      if(!advisorRows.length){
        throw new Error('No advisor rows found. Please use the Kia Service Ranker Excel export.')
      }

      setImportStatus(`Importing ${advisorRows.length} advisor rows...`)
      await replaceServiceRanker(advisorRows, metrics)
      setImportStatus(`Imported ${advisorRows.length} advisor rows. Store NPS ${metrics.store_nps}, Region ${metrics.region_score}, National ${metrics.national_score}.`)
