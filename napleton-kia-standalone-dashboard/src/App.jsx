...
import './styles.css'
import { getStats, getAdvisorStats, getSurveyRecords, updateSurveyRecord, replaceServiceRanker, replaceDispositionImports, replaceFollowUpImports, saveAdvisorPhoto } from './api'
...
function AdvisorLeaderboard({advisors=[], onPhotoSaved}){
...
function FollowUpTable({records,totalRecords,loading,activeFilter,onFilterChange,onSaved}){
  ...
  const advisorOptions = useMemo(() => {
    const names = Array.from(new Set(records.map(r => String(r.advisor || '').trim()).filter(Boolean)))
  ...
  const filtered = useMemo(()=>{
    ...
    const baseRows = records
      .filter(r => advisorFilter === 'All Advisors' || String(r.advisor || '').trim() === advisorFilter)
    ...
    <aside className="executive-sidebar">
      <div className="sidebar-logo-panel logo-like-reference">
        <div className="logo-row">
          <div className="kia-outline-logo">KIA</div>
          <div className="logo-divider" />
          <div>
            <div className="napleton-script">Napleton&apos;s</div>
            <div className="sidebar-subtitle">Mid Rivers Kia</div>
          </div>
        </div>
      </div>
      ...
      <div className="sidebar-user">
        <div className="user-circle">BG</div>
        <div>
          <b>Billy Greenhaw</b>
...
function ManagementPanel({ csiFileRef, dispositionFileRef, followUpFileRef, handleCsiUpload, handleDispositionUpload, handleFollowUpUpload, importStatus, refreshDashboard }) {
  return (
    <section className="management-grid">
      <div className="panel management-panel">
        <div className="panel-head">
          <div>
            <div className="panel-title"><Users size={17}/> Management Tools</div>
            <div className="small">Use this area to update dashboard data from Kia exports.</div>
          </div>
        ...
        <div className="upload-card-grid">
          <div className="upload-card">
            ...
            <h3>CSI Report</h3>
            <p>Updates Store NPS, Region NPS, National NPS, and the Advisor Leaderboard.</p>
          ...
          <div className="upload-card">
            ...
            <p>Imports the Kia Service Invitation List and checks who is missing follow-up.</p>
          ...
          <div className="upload-card">
            ...
            <p>Imports your survey follow-up tracker and compares it to the disposition report.</p>
      ...
      <div className="panel management-panel">
        <div className="panel-head">
          <div className="panel-title"><Activity size={17}/> Daily Update Order</div>
        </div>
        <div className="step-list">
          <div><span>1</span><b>Upload CSI Report</b><p>Refreshes NPS and advisor scores.</p></div>
          <div><span>2</span><b>Upload Disposition Report</b><p>Refreshes Kia service invitation records.</p></div>
          <div><span>3</span><b>Upload Follow-Up List</b><p>Refreshes follow-up statuses and matching.</p></div>
        </div>
      </div>
    </section>
  )
...
function CsiPerformanceView({ stats, advisors }) {
  return (
    <>
      <KpiCards stats={stats} activeFilter="All" onFilterChange={()=>{}}/>
    ...
    <>
      ...
      <section className="dashboard-grid">
        <div className="dashboard-left">
          <AdvisorLeaderboard advisors={advisors} onPhotoSaved={()=>refresh(filter)}/>
        </div>
    ...
    <div className="executive-main">
      <header className="executive-header">
        ...
        <div className="header-actions">
          <div className="date-control"><CalendarDays size={16}/> May 1, 2026 - Current</div>
          <button className="icon-btn" onClick={()=>refresh(filter)} title="Refresh dashboard"><RefreshCw size={17}/></button>
        </div>
      </header>
      ...
      <main className="container executive-container">
        ...
        {activeTab === 'CSI Performance' && <CsiPerformanceView stats={stats} advisors={advisors}/>}
        {activeTab === 'Advisor Rankings' && <AdvisorLeaderboard advisors={advisors} onPhotoSaved={()=>refresh(filter)}/>}
        {activeTab === 'Follow-Up Center' && <FollowUpTable records={records} totalRecords={total} loading={loading} activeFilter={filter} onFilterChange={changeFilter} onSaved={()=>refresh(filter)}/>}
        ...
        {activeTab === 'Management' && (
          <ManagementPanel
            ...
            refreshDashboard={()=>refresh(filter)}
          />
        )}
      </main>
    </div>
