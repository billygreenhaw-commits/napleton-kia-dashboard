...
export async function getRepairOrders() {
  const { data, error } = await supabase
    .from('repair_orders')
    .select('id, ro, tag, customer, model, year, promise_date, status, advisor, tech, notes, imported_at')
...
export async function getPreInvoices() {
  const { data, error } = await supabase
    .from('pre_invoices')
    .select('id, ro, customer, age, model, year, status_desc, advisor, tech, invoice_amount, ro_estimate, notes, imported_at')
