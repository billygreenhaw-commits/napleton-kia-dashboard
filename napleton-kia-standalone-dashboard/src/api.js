...
async function insertWithSchemaFallback(tableName, rows) {
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const { error } = await supabase.from(tableName).insert(workingRows)
    if (!error) return { removedColumns }

    const msg = String(error.message || error.details || '')
    const match = msg.match(/Could not find the ['\"]([^'\"]+)['\"] column|column ['\"]?([^'\"\s]+)['\"]? does not exist/i)
  ...
  throw new Error(`Import failed because Supabase rejected too many missing columns on ${tableName}: ${removedColumns.join(', ')}`)
}
...
export async function replaceAdvisorSalesSummary(rows) {
  ...
  const allowedColumns = [
    ...
    'labor_gp_percent','labor_sales','labor_gross','month_label','net_elr','parts_gp','parts_gross','parts_gp_percent','parts_sales',
    'cp_labor_gross','cp_parts_gross','warranty_labor_gross','warranty_parts_gross','internal_labor_gross','internal_parts_gross',
    'internal_gross','total_internal_gp','internal_ros','warranty_ros','cp_sold_hours','warranty_sold_hours','internal_sold_hours','cp_labor_sales','warranty_labor_sales','internal_labor_sales',
    'projected_cp_gross','projected_customer_pay_gp','projected_parts_gp','projected_ros','projected_sold_hours',
    'projected_cp_labor_gross','projected_cp_parts_gross','projected_warranty_labor_gross','projected_warranty_parts_gross','projected_internal_labor_gross','projected_internal_parts_gross',
    'projected_internal_gross','projected_total_internal_gp','projected_total_gross','projected_total_warranty_gp','projected_warranty_gp','projected_warranty_gross',
  ...
  let removedColumns = []
  if (dbRows.length > 0) {
    const result = await insertWithSchemaFallback('advisor_sales_summary', dbRows)
  ...
  return {
    ...
    skippedUnknownAdvisor,
    replaced: true,
    warning: removedColumns.length ? `Imported, but Supabase is missing these optional columns: ${removedColumns.join(', ')}. Run advisor_sales_summary_labor_parts_gross_sql.sql to permanently store the full labor/parts split.` : '',
  }
}
