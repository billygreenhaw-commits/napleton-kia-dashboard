...
export async function replaceAdvisorMetricsPerformance(rows) {
  ...
  await safeDeleteAll('advisor_metrics_performance')
...
export async function replaceSurveyResults(rows) {
  const validRows = (rows || []).filter(r => String(r?.ro_number || '').trim())

  await safeDeleteAll('survey_results')

  if (validRows.length > 0) {
    const { error } = await supabase.from('survey_results').insert(validRows)
    if (error) throw error
  }

  return {
    imported: validRows.length,
    skippedNoRo: (rows || []).length - validRows.length,
    skippedUnknownAdvisor: 0,
    replaced: true,
  }
}
...
export async function replaceAdvisorSalesSummary(rows) {
  ...
  if (cleanRows.length > 0) {
    const { error } = await supabase.from('advisor_sales_summary').insert(cleanRows)
    if (error) throw error
  }
...
export async function getSurveyResults() {
  ...
  return (data ?? []).filter(row => String(row?.ro_number || '').trim())
}
...
export async function replaceProgramHealthMetrics(rows) {
  ...
  if ((rows || []).length > 0) {
    const { error } = await supabase.from('program_health_metrics').insert(rows)
    if (error) throw error
  }
