...
function buildSurveyResultMap(results = []) {
  ...
  results.forEach((s) => {
    ...
    if (name) byName.set(`name:${name}`, s)
  })
...
function findSurveyResult(row, maps) {
  ...
  if (name && maps.byName.has(`name:${name}`)) return maps.byName.get(`name:${name}`)
  return null
}
...
export async function getStats() {
  ...
  const { data: surveyResults, error: sErr } = await supabase.from('survey_results').select('*')
  if (sErr) throw sErr
  ...
  const surveyKeys = new Set((surveyResults ?? []).map(getRowKey))
  ;(followups ?? []).forEach((f) => {
    if (surveyKeys.has(getRowKey(f))) counts.took_survey++
  })
