...
function getRowKey(row) {
  ...
  if (ro) return `ro:${ro}`
  return `name:${normName(row.customer_name)}|advisor:${normName(row.advisor)}`
}
...
export async function getStats() {
  ...
  const followupKeys = new Set((followups ?? []).map(getRowKey))
  ;(dispositions ?? []).forEach((d) => {
    if (followupKeys.has(getRowKey(d))) counts.matched_total++
...
export async function getSurveyRecords(status = 'All') {
  ...
  ;(dispositions ?? []).forEach((d) => dispositionMap.set(getRowKey(d), d))

  const usedDispositionKeys = new Set()

  const enrichedFollowups = (followups ?? []).map((r) => {
    const key = getRowKey(r)
    const match = dispositionMap.get(key)
    if (match) usedDispositionKeys.add(key)
