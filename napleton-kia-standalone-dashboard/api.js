...
export async function replaceAdvisorSalesSummary(rows) {
  const cleanRows = rows
    ...
    .map((r) => ({
      ...
      advisor: r.advisor || null,
      ros: r.ros == null ? null : Number(r.ros),
      sold_hours: r.sold_hours == null ? null : Number(r.sold_hours),
      warranty_gp: r.warranty_gp == null ? null : Number(r.warranty_gp),
      parts_gp: r.parts_gp == null ? null : Number(r.parts_gp),
      total_warranty_gp: r.total_warranty_gp == null ? null : Number(r.total_warranty_gp),
      labor_gp_percent: r.labor_gp_percent == null ? null : Number(r.labor_gp_percent),
      parts_gp_percent: r.parts_gp_percent == null ? null : Number(r.parts_gp_percent),
      gp_percent: r.gp_percent == null ? null : Number(r.gp_percent),
      customer_pay_gp: r.customer_pay_gp == null ? null : Number(r.customer_pay_gp),
      elr: r.elr == null ? null : Number(r.elr),
      avg_cp_hours_ro: r.avg_cp_hours_ro == null ? null : Number(r.avg_cp_hours_ro),
      avg_hours_ro: r.avg_hours_ro == null ? null : Number(r.avg_hours_ro),
      projected_ros: r.projected_ros == null ? null : Number(r.projected_ros),
      projected_sold_hours: r.projected_sold_hours == null ? null : Number(r.projected_sold_hours),
      projected_warranty_gp: r.projected_warranty_gp == null ? null : Number(r.projected_warranty_gp),
      projected_parts_gp: r.projected_parts_gp == null ? null : Number(r.projected_parts_gp),
      projected_total_warranty_gp: r.projected_total_warranty_gp == null ? null : Number(r.projected_total_warranty_gp),
      projected_customer_pay_gp: r.projected_customer_pay_gp == null ? null : Number(r.projected_customer_pay_gp),
    }))
  ...
  if (cleanRows.length > 0) {
    const { error: insertError } = await supabase.from('advisor_sales_summary').insert(cleanRows)
    if (insertError) throw insertError
  }
...
export async function replacePaceMetrics(rows) {
  const cleanRows = rows
    ...
    .map((r) => ({
      ...
      metric_label: r.metric_label || r.metric_key,
      metric_value: r.metric_value == null ? null : Number(r.metric_value),
      metric_display: r.metric_display || null,
      section: r.section || 'PACE',
    }))
  ...
  if (cleanRows.length > 0) {
    const { error: insertError } = await supabase.from('pace_metrics').insert(cleanRows)
    if (insertError) throw insertError
  }
